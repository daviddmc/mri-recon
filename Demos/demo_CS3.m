...
%% setup operators
ft = FourierTransform('1', 0, [1 2]);
E = Undersampling(ft, 0, mask_vardens, 0);
wt = WaveletTransform('1', 0, 'Daubechies',4,4);
...
solver = ALM({ssd, tv, l1norm}, {eq1, eq2}, param);

%% run solver
[x, info] = solver.run({im_zf, G.apply(im_zf), wt.apply(im_zf)});
