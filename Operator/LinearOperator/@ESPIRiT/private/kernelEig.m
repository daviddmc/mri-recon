function [EigenVecs, EigenVals] = kernelEig(kernel, imSize, F)
...
for n=1:size(kernel,4)
    tmp = padCenter(conj(kernel(end:-1:1,end:-1:1,:,n)) * sqrt(imSize(1)*imSize(2)), [imSize(1), imSize(2)]);
    KERNEL(:,:,:,n) = F.apply(tmp);
end
