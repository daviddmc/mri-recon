...
I = Identity([]);
W = Weighting([], 0, w);
Fw = FourierTransformation(W, 1, [1,2]); % precondition operator for sensitivity maps
bd = BlockDiagonal('1', {I, Fw}, {1, 2:nc+1});
% Jacobian of nonlinear operator J
C = SensitivityMap([], 0, im_zf ./ rootSumSquare(im_zf), 0);
Rho = Weighting([], 0, rootSumSquare(im_zf));
...
for iter = 1 : nIterOut
    % setup solver
    lq = LSQR({Jac, []}, {resDC, resL2Reg}, [1, sqrt(alpha)]);
    % run solver
    [z, info] = lq.run(XN, param);
    % update variable
    XN = XN + z;
...
%% show results
im_recon = XN(:, :, 1) .* rootSumSquare(cmap) / yscale;
imshow(abs(im_recon), []);
