...
kCalib = cropCenter(kData,[CalibSize,nc]); % crop center k-space for calibration
...
lq = LSQR({GFFT_u, GOP}, {b, 0 * im_zf}, [1, 1]);

%% run solver
param.maxIter = nIterCG;
param.verbose = 2;
im_spirit = lq.run(im_zf, param);

%% show results
imshow(cat(2,rootSumSquare(im_gt),rootSumSquare(im_zf),rootSumSquare(im_spirit)),[]), 
