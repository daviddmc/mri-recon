...
[sx,sy,Nc] = size(DATA);
invF = FourierTransformation([], 1, [1,2]);
im_gt = invF.apply(DATA);
DATA = DATA / max(abs(im_gt(:))) + eps;
im_gt = im_gt / max(abs(im_gt(:)));
mask = padCenter(ones(sx, ncalib),[sx, sy]);
mask = repmat(mask,[1,1,8]);
mask(:,1:2:end,:) = 1;
DATAc = DATA .* mask;
calib = cropCenter(DATAc,[sx,ncalib]);
...
lq = LSQR({A, []}, {b, 0*x0}, [1, lambda]);

%% run solver
param.maxIter = 12;
param.verbose = 2;
[x, info] = lq.run(x0, param);
...
subplot(1,3,3), imshow(rootSumSquare(im_espirit)), title('ESPIRiT');
