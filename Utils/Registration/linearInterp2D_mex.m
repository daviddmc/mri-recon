...
function Output_images = linearInterp2D_mex(Input_images, XI,YI, isT)

pathtofile=which('linearInterp2D_mex.cpp');
pathstr = fileparts(pathtofile);
mex(pathtofile,'-outdir',pathstr);

if nargin == 4
    Output_images = linearInterp2D_mex(Input_images, XI,YI, isT);
else
    Output_images = linearInterp2D_mex(Input_images, XI,YI, 0);
end
