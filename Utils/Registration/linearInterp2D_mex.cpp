...
)
{
    ...
    for (n=0; n < MN; n++) {
        ...
        if (fs < 1 || s > ncols || ft < 1 || t > nrows){/* Put zeros if outside*/
            for (i = 0; i < ndim; i++) F[n+i*MN] = 0;  }
        else  {
            ndx = ft+(fs-1)*nrows;
...
)
{
    ...
    for (n=0; n < MN; n++) {
        ...
        if (fs < 1 || s > ncols || ft < 1 || t > nrows){/* Put zeros if outside*/
            ...
            for (i = 0; i < ndim; i++){
                Zshift=i*nrowsncols;
                F[in1 + Zshift] += m1 * zval;
                F[in2 + Zshift] += m2 * zval;
                F[in3 + Zshift] += m3 * zval;
                F[in4 + Zshift] += m4 * zval;
            }
...
{
    double *Z, *S, *T, *F;
    int  N, M, MN, nrows, ncols, vol, ndim, isTransposed; 
    size_t* newdims;
    const size_t *dims;
   ...
   /* Get the sizes of each input argument */
    ...
    newdims = (size_t*) calloc(ndim, sizeof(size_t));
  ...
  /* Do the actual computations in a subroutine */
    if(isTransposed){
        LinearInterpTrans2D(Z, S, T, F, MN, nrows, ncols, vol);
    }else{
        LinearInterp2D(Z, S, T, F, MN, nrows, ncols, vol);
    }
